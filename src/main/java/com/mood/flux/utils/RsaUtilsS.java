...
public class RsaUtilsS {
    private static final Logger LOGGER = LoggerFactory.getLogger(RsaUtilsS.class);
    ...
    private static String getKeyString(String keyName,String fileName){
        if (RsaUtilsS.class.getClassLoader().getResource(fileName) == null){
            LOGGER.warn("getKeyString()# " + fileName + " is not exist.Will run #generateKeyPair()# firstly.");
            generateKeyPair();
        }
        try(InputStream in = RsaUtilsS.class.getClassLoader().getResource(fileName).openStream()){
            Properties properties = new Properties();
            properties.load(in);
